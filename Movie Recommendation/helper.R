...
library(reshape2)
movies <- read.csv("C:/Users/Dhruba/Desktop/GitHub/R - GitHub/data/movies.csv", header = TRUE, stringsAsFactors=FALSE)
...
movie_recommendation <- function(input,input2,input3) {
  ...
  #input2 = "Father of the Bride Part II (1995)"
  #input3 = "Heat (1995)"
  ...
  if (as.character(recom_list[1])=='character(0)'){
    ...
    colnames(recom_result) <- "DSRC Recommends these movies"
    return(recom_result)
  }
}
